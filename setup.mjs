...
async function main() {
  ...
  if (!/[<>]/.test(url)) env.OOG_URL = url; // persist so `npm start` can print a scan-to-connect QR
  ...
  if (!/[<>]/.test(url)) {
    const ok = await printQR(`${url}/?token=${encodeURIComponent(token)}`);
    if (ok) log(`📱 Scan the code above on your phone — it opens already signed in (no token typing).`);
  }
  log(`Token (if you'd rather type it): ${C.b}${token}${C.x}`);
