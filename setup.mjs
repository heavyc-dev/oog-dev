...
async function main() {
  ...
  if (mode === 0) {
    ...
    if (ip.status === 0) {
      ...
      log(`Then open the ${C.b}https://<your-pc>.<tailnet>.ts.net${C.x} URL on your phone (see: tailscale serve status).`);
    ...
    env._URL = "https://<your-pc>.<tailnet>.ts.net";
  } else {
    head("Local HTTP");
    env.PORT = "8765";
  ...
  // 7) write .env
  const url = env._URL; delete env._URL;
